 ...
 *
 * Ogni collezione dell'archivio (film, spettacoli, prenotazioni…) sta in una
 * riga della tabella `archivio`, con il contenuto in una colonna `jsonb`. È una
 * scelta deliberata per questa fase: il pannello lavora sempre sull'insieme
 * completo di una collezione, e i tipi in `lib/tipi.ts` restano l'unico schema
 * da tenere aggiornato.
 *
 * Il limite da conoscere, e da superare quando la catena cresce: `jsonb` non
 * impone vincoli fra collezioni e non permette query per singolo campo dal
 * database. `docs/schema-database.md` contiene lo schema relazionale
 * normalizzato corrispondente, con le tabelle da creare quando `prenotazioni`
 * e `spettacoli` diventeranno troppo grandi per essere caricate per intero —
 * sono le due che lo giustificano per prime.
 *
      ...
      try {
        ...
        for (const collezione of COLLEZIONI) {
          await connessione.query(
            ...
            // `impostazioni` è un oggetto, tutte le altre collezioni sono
            // elenchi: `?? null` copre entrambi i casi senza trasformare un
            // oggetto mancante in un array vuoto.
            [collezione, JSON.stringify(dati[collezione] ?? null)],
          )
        }
